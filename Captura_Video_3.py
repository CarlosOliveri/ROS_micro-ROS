...
blueBajo = np.array([105,150,200],np.uint8)#robot => azul[100,100,50][140,255,100]
blueAlto = np.array([130,255,255],np.uint8)
#Limite de los obstaculos
#obstBajo = np.array([0,27,150],np.uint8) # obstaculos => rojo
#obstAlto = np.array([5,255,255],np.uint8)
obstBajo = np.array([165,25,150],np.uint8) # obstaculos => rojo  165,27,150  180,255,255
obstAlto = np.array([180,255,255],np.uint8)

# Parámetros de calibración de la cámara
# Carga directa de los parámetros de calibración
try:
...
def distancia_entre_coord(a,b):
  dist_min = math.sqrt(nw**2 + nh**2)/2.5#1.8
...
class Grafo:
  ...
  def agregar_vertice(self,vertice):
    ...
    for k in elementos_obstaculos:
      if distancia_entre_coord(k,vertice):
        if vertice.clave[0] < (k[0] + k[2]) and (vertice.clave[0] ) > k[0]:       ##CAMBIAMMOS#############################3
  ...
  def menor_trayecto(self,pos):
    ...
    print('-------------------------------')
    for k in min_tray:
       print(str(k.clave[0]*lado_robot/nw)+","+str(k.clave[1]*lado_robot/nh))
    #return min_tray
...
while True:
    try:
        ...
        # Supongamos que `frame` es la imagen que quieres recortar
        # Establece los recortes en píxeles para los laterales
        ...
        # Factor de escala
        #scale_factor = 1  # Redimensiona al 50% del tamaño original
        ...
        for j in range(len(elementos_obstaculos)):
          #Obstaculost
          cv2.line(frame,(elementos_obstaculos[j][0],elementos_obstaculos[j][1]),(elementos_obstaculos[j][0]+elementos_obstaculos[j][2],elementos_obstaculos[j][1]),(0, 0, 0), 4)
          cv2.line(frame,(elementos_obstaculos[j][0],elementos_obstaculos[j][1]),(elementos_obstaculos[j][0],elementos_obstaculos[j][1]+elementos_obstaculos[j][3]),(0, 0, 0), 4)
          cv2.line(frame,(elementos_obstaculos[j][0],elementos_obstaculos[j][1]+elementos_obstaculos[j][3]),(elementos_obstaculos[j][0]+elementos_obstaculos[j][2],elementos_obstaculos[j][1]+elementos_obstaculos[j][3]),(0, 0, 0), 4)
          cv2.line(frame,(elementos_obstaculos[j][0]+elementos_obstaculos[j][2],elementos_obstaculos[j][1]+elementos_obstaculos[j][3]),(elementos_obstaculos[j][0]+elementos_obstaculos[j][2],elementos_obstaculos[j][1]),(0, 0, 0), 4)
